...
def write_html(html_error):
    ...
    container = str(soup.find(id='container').find_all(
        "div", {"class": "line"}))
    h2_line = str(soup.find(id="container").find("h2"))
    encoded_str = (title+container).encode()
    html_hash = hashlib.md5(encoded_str).hexdigest()
    short_hash = str(html_hash[0:5])
    print(style("Backend Error: {}. {}. See html file".format(title, h2_line), Fore.RED))
...
def line_loop(line):
    ...
    if line[0] == "{":
        ...
        log_level = style(log_level.upper(), cols[log_level])
        time_log = json_data.pop("meta_time_logged")
        line_str += style(time_log + " ", cols["time"])
...
def print_loop(log_file, log_size_threshold):
    ...
    same_log_file = prev_log_file and prev_log_file == log_file
    ...
    retry_print_loop_after_secs = 15
    start_time = time.time()
    start_logging = not same_log_file
    while True:
        if p.poll(1):
            ...
            if start_logging:
                ...
                start_logging = prev_log_line.strip() in line.decode()
        else:
            curr_time = time.time()
            delta = (curr_time - start_time)
            if delta > check_size_after_secs:
